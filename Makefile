...
VALGRIND_FLAGS := --leak-check=full --show-leak-kinds=all --leak-resolution=med --track-origins=yes
...
format:
	...
	@find $(SRC_DIR) -type f \( -name "*$(SOURCE_EXTENSION)" -o -name "*.h" \) -exec clang-format -i --style=$(FORMAT_STYLE) {} \;

check_format:
	@find $(SRC_DIR) -type f \( -name "*$(SOURCE_EXTENSION)" -o -name "*.h" \) -exec clang-format -n --style=$(FORMAT_STYLE) {} \; -exec printf "Check format \e[$(MAIN_COLOR)m{}$<\e[0m\n" \;
